...
eval "$(jq -r '@sh "PCC_URL=\(.pcc_url) PCC_USER=\(.pcc_user) PCC_PASS=\(.pcc_pass) FILENAME=\(.filename) RUNTIME=\(.runtime) MODULE_TYPE=\(.module_type)"')"
...
curl -k -H "Content-Type: application/json" -H "Authorization: Bearer $TOKEN" -s "$PCC_URL/api/v1/images/twistlock_defender_layer.zip" -o "$FILENAME" -d '{"runtime":"'$RUNTIME'","provider":"aws", "nodeJSModuleType": "'$MODULE_TYPE'"}'
